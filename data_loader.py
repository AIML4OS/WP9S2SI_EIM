...
def load_input_data(server, database, table, status_zapisa,
    ...
    try:
        ...
        if check: 
            correct_data = pd.concat(all_test_check, ignore_index=True)
            return correct_data, input_data
        else: 
         return input_data
...
def load_input_data_dupl(server, database, table, status_zapisa,
                    id_var, schema,
                    trusted_connection='yes', chunksize=50000,
                    parquet_path=None, preveri_test=False):
    """
    Load data from a SQL Server table (including _edi), selecting in _edi only rows with status_zapisa we want to work on.
    - Keeps only the latest record per id_var using ROW_NUMBER().
    - Rounds all float values to 2 decimals.
    ...
    try:
        ...
        with pyodbc.connect(conn_str) as conn:
            ...
            for i, chunk in enumerate(chunks):
                ...
                if preveri_test: 
                    double = chunk[chunk["STATUS_ZAPISA"] == status_zapisa]
                    first = chunk[chunk["STATUS_ZAPISA"] == 1]
                    double_id = double[id_var]
                    first_double = first[first[id_var].isin(double_id)]
                    all_test_check.append(first_double)
                    clean_first = first[~first[id_var].isin(double_id)]
                    clean_lst = [clean_first, double]
                    chunk = pd.concat(clean_lst)
                all_chunks.append(chunk)
        ...
        if preveri_test: 
            check_data = pd.concat(all_test_check, ignore_index=True)
            return check_data, input_data
        else: 
         return input_data
