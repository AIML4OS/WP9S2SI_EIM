...
def load_input_data(server, database, table, status_zapisa,
                    ...
                    parquet_path=None):
    """
    Load data from a SQL Server table (including _edi), selecting in _edi only rows with status_zapisa we want to work on.
    ...
    - status_zapisa: record version ( 1 in base table; 2, 3, 4, 5... - in _edi table) on which we want to work
    - id_var: ID variable
    - schema: Schema name
...
def load_input_data_more_versions(
    server,
    database,
    table,
    status_zapisa,               # used only if 'statuses' is not provided
    id_var,
    schema,
    trusted_connection='yes',
    chunksize=50000,
    statuses: Optional[Iterable[int]] = None,
    create_vars: bool = True,
    keep_only_max_status: bool = False,  #  keep only rows with the maximum status_zapisa in each df
) -> Dict[int, pd.DataFrame]:
    
    """
    Load data for one or multiple 'status_zapisa' values from the base table and the _edi table.

    Logic:
      - Base table [{schema}].[{table}] is filtered to status_zapisa = 1 (as per the EARN rule).
      - EDI table [{schema}].[{table}_edi] is filtered to each requested status.
      - UNION ALL the two sources; then, for each {id_var}, keep only the latest record based on
        ROW_NUMBER() OVER (PARTITION BY {id_var} ORDER BY status_zapisa DESC).
      - For each status 's' return one DataFrame (optionally also create a global variable data_<s>).
      - If keep_only_max_status=True, filter the resulting DataFrame to rows that have the maximum
        status_zapisa present in that DataFrame (e.g., keep only 41 if both 1 and 41 are present).

    Note: expects your helper functions to be available in scope:
      - round_all_float_columns(df, decimals=2)
      - transform_dataframe(df, keep_float64=list_of_float_cols)
    """
    # If no explicit list of statuses is provided, fall back to the single 'status_zapisa' parameter
    if statuses is None:
        statuses = [status_zapisa]

    # Build connection string (same driver style as in your original function)
    conn_str = (
        f"DRIVER={{SQL Server}};"
        f"SERVER={server};"
        f"DATABASE={database};"
    ...
    with pyodbc.connect(conn_str) as conn:
        for s in statuses:
            ...
            # Base (status=1) + EDI (status=s); then keep latest per id_var
            query = f"""
            SELECT *, FORMAT({id_var}, '000000000') AS ident
            FROM (
                SELECT *,
                       ROW_NUMBER() OVER (
                           PARTITION BY {id_var}
                           ORDER BY status_zapisa DESC
                       ) AS rn
                FROM (
                    SELECT * FROM {schema}.{table}
                    WHERE status_zapisa = 1

                    UNION ALL

                    SELECT * FROM {schema}.{table}_edi
                    WHERE status_zapisa = {s}
                ) AS combined
            ) AS ranked
            WHERE rn = 1;
            """
            ...
            for i, chunk in enumerate(chunks, start=1):
                # Round float columns to 2 decimals
                chunk = round_all_float_columns(chunk, decimals=2)
                # Identify float64 columns to keep as float64
                float_cols = chunk.select_dtypes(include=['float']).columns.tolist()
                # Optimize remaining dtypes
                chunk = transform_dataframe(chunk, keep_float64=float_cols)

                print(f"[{s}] chunk {i}: {chunk.shape[0]} rows, "
                      f"~{chunk.memory_usage(deep=True).sum()/1e6:.2f} MB")
                if i == 1:
                    print("Preview first chunk:")
                    print(chunk.head())
                processed.append(chunk)

            df_s = pd.concat(processed, ignore_index=True) if processed else pd.DataFrame()

            # --- Keep only rows with the maximum status_zapisa (optional) ---
            if keep_only_max_status and not df_s.empty:
                ...
                status_col = _col(df_s, "status_zapisa")

                # coerce to numeric to avoid string comparisons ("41" > "7" issues)
                status_num = pd.to_numeric(df_s[status_col], errors="coerce")
                max_status = int(np.nanmax(status_num.to_numpy()))

                # keep only rows with the maximum status in this DataFrame
                df_s = df_s[status_num == max_status].copy().reset_index(drop=True)

                print(f"[{s}] keep_only_max_status=True ⇒ kept only rows with max {status_col}={max_status} "
                    f"({df_s.shape[0]} rows).")

            print(f"✅ [{s}] Loaded {df_s.shape[0]} rows in {time.time() - t0:.2f}s")

            # Store in results dict
            results[s] = df_s

            # Optionally create a global variable data_<status> in the caller's globals
            if create_vars:
                caller_globals = inspect.currentframe().f_back.f_globals
                var_name = f"data_{s}"
                caller_globals[var_name] = df_s
                print(f"🔗 Created variable '{var_name}'")

    print(f"\nDone. Total time: {time.time() - start_all:.2f}s")
    return results
